...
class BerandaController extends Controller
{
    ...
    public function konfirmasa(Request $request)
    {
        ...
        $RUMA = DB::select('CALL referensi(?)', ['RUMAH']);
        ...
        // $client = new Client();
        // $response = $client->post('http://172.21.20.88:8000/api/cekKtp', [
        ...
        try {
            ...
            // Jika terjadi Exception saat mengirim permintaan, kembalikan ke halaman sebelumnya dengan pesan error
            // return redirect()->view('unggahfoto')->withErrors(['error' => 'Terjadi kesalahan']);
            return view('unggahfoto')->with('error', 'Terjadi kesalahan size melebihi 2MB atau tidak sesuai format');
        }
        ...
        if (is_null($dataObj)) {
            ...
            if ($jumlah == 0) {
                $responses = $client->request('POST', 'http://172.21.20.88:8000/api/inquirybyNik', [
                    'form_params' => [
                        'noKtp' => $dataObj['idNumber'],
                    ]
                ]);
                ...
                if ($responseBody['cifNo'] == "NN") {
                    ...
                    } elseif ($dataObj['occupation'] == "PELAJAR/MAHASISWA") {
                        $kerja = "LAIN LAIN";
                    } else {
                        $kerja = $dataObj['occupation'];
                    }

                    $dataxkrj = DB::select('select kode from referensi where keterangan = ? and tipe = "001";', [$kerja]);
                    ...
                    return view('konfirmasi', $array, compact('prop', 'agam', 'STATUSNIKA', 'RUMA'));
                } else {
                    echo " <script>
    ...
    public function datapekerjain(Request $request)
    {
        ...
        if ($btnkrmdata == "Kembali") {
            ...
            $isidata =  $regid . "|" . $nomornik . "|" . $namaCust . "|" . $gelar . "|" . $address1 . "|" . $address2 . "|" . $address3 . "|" . $address4 . "|" . $address5
                ...
                . "|" . $statusrumah . "|" . $motherMaidenName . "|" . $email . "|" . $dom1 . "|" . $nohp . "|" . $hobby ;
            ...
            return view('datapekerjaan', $array, compact('prop', 'PEKERJAA', 'BIDANGPEKERJAA', 'JABATA', 'KARYAWA', 'RUMA'));
        }
    } catch (MethodNotAllowedHttpException $e) {
        // Tangani pengecualian MethodNotAllowedHttpException di sini
    ...
    public function datakeuangin(Request $request)
    {
        ...
        if ($btnkrmdata == "Kembali") {
            ...
            if($jobPositionCode == "27"){
                $bidangusaha = "27";
                $jabatan = $request->input('jabatans');
            }else{
            ...
            return view('datakeuangan', $array, compact('SUMBERPENGHASILA', 'SUMBERDAN', 'PENDAPATA', 'RUMA'));
        }
    }
    ...
    public function getData($id)
    {
        ...
        $data = DB::select('select * from tbl_wilayah where LENGTH(kode)=5 and LEFT(kode,2) = ?', [$id]);

        // Kembalikan data dalam format JSON
        return response()->json($data);
    }

    public function getKabupaten($propinsiKode)
    {
        $kabupaten = DB::select('SELECT DISTINCT kode, nama FROM tbl_wilayah WHERE LENGTH(kode) = 5 AND LEFT(kode, 2) = ? order by nama', [$propinsiKode]);
        return response()->json($kabupaten);
    }

    public function getKecamatan($kabupatenKode)
    {
        $kecamatan = DB::select('SELECT kode, nama FROM tbl_wilayah WHERE LENGTH(kode) = 8 AND LEFT(kode, 5) = ? order by nama', [$kabupatenKode]);
        return response()->json($kecamatan);
    }
    public function getKelurahan($kecamatanKode)
    {
        $kelurahan = DB::select('SELECT kode, nama FROM tbl_wilayah WHERE LENGTH(kode) = 13 AND LEFT(kode, 8) = ?', [$kecamatanKode]);
